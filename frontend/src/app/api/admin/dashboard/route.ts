...
export async function GET() {
  try {
    ...
    const [ordersRes, usersRes, lowStockRes, analyticsRes, mailRes] = await Promise.allSettled([
      fetch(`${ORDER_SERVICE_URL}/api/v1/admin/orders?page=0&size=5&sort=createdAt,desc`, { headers: authHeaders }),
      fetch(`${USER_SERVICE_URL}/api/v1/admin/users?page=0&size=5`, { headers: authHeaders }),
      fetch(`${INVENTORY_SERVICE_URL}/api/v1/inventory/low-stock?threshold=10`, { headers: authHeaders }),
      fetch(`${AI_SERVICE_URL}/api/v1/analytics/dashboard?dashboardType=overview`, { headers: authHeaders }),
      fetch(`${MAIL_SERVICE_URL}/api/v1/mail/logs?page=0&size=5`, { headers: authHeaders }),
    ]);
