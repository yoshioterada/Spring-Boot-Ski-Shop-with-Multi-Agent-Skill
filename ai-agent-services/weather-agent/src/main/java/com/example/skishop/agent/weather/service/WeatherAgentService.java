...
public class WeatherAgentService {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            **重要な制約**:
            - 各ツールは指定された場所について **1 回だけ** 呼び出すこと。同じツールを別の言い換え（"Naeba" と "苗場" 等）で何度も呼んではならない。
            - 場所名のバリエーション（英語・日本語・別表記）について別ツール呼び出しを行ってはならない。最初に与えられた location をそのまま使用すること。
            - 全ツールの呼び出しが終わったら、追加のツール呼び出しを行わずに直ちに最終回答を返す。
