...
備考: i18n のハードコード文字列の完全移行は約50+ファイルに影響する大規模タスクです。ja.json にキーは追加済みのため、各ファイルで import { t } from '@/lib/i18n' して置換する作業を段階的に進められます。代表的なファイル（ec-header, ec-footer, admin-sidebar, confirm-dialog, pagination, catalog）は既に移行済みです。